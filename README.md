...
This application loads data from separate JSON files, which requires an HTTP server:
...
To update sample data, edit the JSON files in the `data/` directory. Each file is a JSON array of objects. The application loads these files on startup -- no code changes are needed.
...
- **Architecture:** Modular JavaScript (IIFE pattern, no build step required)
